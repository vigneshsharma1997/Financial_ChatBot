...
class MySqlConnection(object): 
    ...
    def insert(self,session_id,chat_id,source_agent,user_query,question_title): 
        try: 
            print("inside Insert().") 
            self.connect() 
            self.cursor.execute(insert_chat_query,(session_id,chat_id,source_agent,user_query,question_title)) 
            self.conn.commit() 
...
def insert_session_id_chat_id_to_db(payload): 
    try : 
        ...
        db.insert(payload["session_id"],payload["chat_id"],payload["source_agent"],payload["user_query"],payload["question_title"]) 
        return True 
    except Exception as e: 
        return False       
